...
public class MainPane extends BorderPane {
	...
	private void myHandle(ActionEvent e) {
		...
		if (result == 0)
			...
			this.controller.setCurrentCellItem(result, result, null);
			for (int row = 0; row < controller.getSize(); row++)
                for (int column = 0; column < controller.getSize(); column++)
                    if (!controller.getCurrentCellItem(row, column).equals(controller.getSolutionCellItem(row, column)))
                        if (!controller.getCurrentCellItem(row, column).equals(ShipItem.EMPTY))
                            controller.setCurrentCellItem(row, column, ShipItem.EMPTY);
