...
public class MainPane extends BorderPane {
	...
	public MainPane(Controller controller) {
		...
		populateCombo(comboFrom);
		comboTo   = new ComboBox<>(); 
		populateCombo(comboTo);
	...
	private void showSorted(OrderType orderType) {
		String fromString = comboFrom.getValue();
		String toString = comboTo.getValue();
		List<Route> listRoutes = this.controller
				.getRoutes(fromString, toString);
		Comparator<Route> cmp = 
				orderType == OrderType.BY_DISTANCE ? Comparator.comparingDouble(Route::getLength) 
												   : Comparator.comparing(controller::getRouteDuration);
		listRoutes.sort(cmp);
		
		outputArea.setText(listRoutes.stream()
				.map(this::toRouteString)
				.collect(Collectors.joining(System.lineSeparator()+System.lineSeparator())));
	}
	
	private String toRouteString(Route route) {
		Duration duration = controller.getRouteDuration(route);
		long minutes = duration.toMinutes();

		return route.toString() + System.lineSeparator() +  
				"Durata: " + String.format("%d:%02d", minutes / 60, (minutes % 60));
	}
