...
public class MyPollPane extends BorderPane {
	...
	public MyPollPane(Controller controller) {
		...
		ageMin.setPrefWidth(300);	 
		ageMax.setPrefWidth(300);
		//ageMin.setShowTickMarks(true);
		ageMin.setShowTickLabels(true); 
		ageMax.setShowTickLabels(true);
		ageMin.setMajorTickUnit(10);  
		ageMax.setMajorTickUnit(10);
		ageMin.setSnapToTicks(true); 
		ageMax.setSnapToTicks(true);
		ageMin.setBlockIncrement(1); 
		ageMax.setBlockIncrement(1);
		...
		tg = new ToggleGroup();
		entrambi = new RadioButton("entrambi");	
		m = new RadioButton(Sesso.MASCHIO.toString());  
		f = new RadioButton(Sesso.FEMMINA.toString());	
		entrambi.setToggleGroup(tg);
		m.setToggleGroup(tg);
		f.setToggleGroup(tg);
