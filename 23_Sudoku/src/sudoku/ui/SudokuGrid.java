...
public class SudokuGrid extends GridPane {
	...
	private void insertNumber(int row, int col) {
		...
		boolean set = controller.setCell(row, col, value);
		if (!set) {
			Alert alert = new Alert(AlertType.ERROR);
			alert.setTitle("Attenzione");
			alert.setHeaderText("Operazione non consentita");
			alert.setContentText("Non è possibile inserire il numero in quella posizione");
			alert.showAndWait();
			return;
		}
		...
		if (controller.endGame()) {
			...
			alert.setContentText("Sudoku terminato con successo");
			alert.showAndWait();
			controller.saveGame();
			return;
		}
